    ...
    <div class="navbar navbar-default navbar-fixed-top" role="navigation">
          <div class="container">
            ...
            <div class="navbar-collapse collapse">
              <!-- 获取当前的模块操作 -->
              <?php  $controller=explode('/','/Admin/Web/listall/status/1/type/0')[2];?>
...
<div class="main-container">
      <div class="projects-header page-header">
          <h3 class="clearfix">
              ...
              <a class="btn btn-primary pull-right" href="/Admin/Web/addinfo" role="button">
...
<div class="alert alert-info">
  <label>状态：</label>
   <?php if($status==''||$status==null||$status==3): ?><a href="/Admin/Web/listall/status/3/type/0" class="btn btn-info">
        <?php else: ?> <a href="/Admin/Web/listall/status/3/type/0" class="btn btn-link"><?php endif; ?>
        全部 </a>
   <?php if($status==0&&$status!=''&&$status!=null): ?><a href="/Admin/Web/listall/status/0/type/0 " class="btn btn-info">
        <?php else: ?> <a href="/Admin/Web/listall/status/0/type/0" class="btn btn-link"><?php endif; ?>
      <i class="icon-spinner"></i> 审核 </a>
    <?php if($status==1 ): ?><a href="/Admin/Web/listall/status/1/type/0" class="btn btn-info">
        <?php else: ?> <a href="/Admin/Web/listall/status/1/type/0" class="btn btn-link"><?php endif; ?>
      <i class="icon-unlock"></i> Enable </a>
    <?php if($status==2 ): ?><a href="/Admin/Web/listall/status/2/type/0" class="btn btn-info">
        <?php else: ?> <a href="/Admin/Web/listall/status/2/type/0" class="btn btn-link"><?php endif; ?>
      <i class="icon-lock"></i> Disable</a>
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  <label>类型：</label>
   <?php if($paramType==0 ): ?><a href="/Admin/Web/listall/status/<?php echo $status;?>/type/0" class="btn btn-info">
        <?php else: ?> <a href="/Admin/Web/listall/status/<?php echo $status;?>/type/0" class="btn btn-link"><?php endif; ?>
        全部 (<?php echo ($typeCount["allCount"]); ?>)</a>
   <?php if($paramType==1 ): ?><a href="/Admin/Web/listall/status/<?php echo $status;?>/type/1" class="btn btn-info">
        <?php else: ?> <a href="/Admin/Web/listall/status/<?php echo $status;?>/type/1" class="btn btn-link"><?php endif; ?>
      <i class="icon-film"></i> 视频 (<?php echo ($typeCount["videoCount"]); ?>) </a>
    <?php if($paramType==2 ): ?><a href="/Admin/Web/listall/status/<?php echo $status;?>type/2" class="btn btn-info">
        <?php else: ?> <a href="/Admin/Web/listall/status/<?php echo $status;?>/type/2" class="btn btn-link"><?php endif; ?>
      <i class="icon-picture"></i> 图片 (<?php echo ($typeCount["imageCount"]); ?>)</a>
    <?php if($paramType==3 ): ?><a href="/Admin/Web/listall/status/<?php echo $status;?>/type/2" class="btn btn-info">
        <?php else: ?> <a href="/Admin/Web/listall/status/<?php echo $status;?>/type/2" class="btn btn-link"><?php endif; ?>
      <i class="icon-book"></i> 文字 (<?php echo ($typeCount["wordsCount"]); ?>)</a>
    <?php if($paramType==4 ): ?><a href="/Admin/Web/listall/status/<?php echo $status;?>/type/4" class="btn btn-info">
        <?php else: ?> <a href="/Admin/Web/listall/status/<?php echo $status;?>/type/4" class="btn btn-link"><?php endif; ?>
      <i class="icon-headphones"></i> 音乐 (<?php echo ($typeCount["musicCount"]); ?>)</a>
</div>
      </div>
  ...
  <?php if($status==0||$status==2): ?><a class="btn btn-default checkbox-opreat disabled" href="javascript:checkChanges('/Admin/Web/checkChanges?status=1','确定批量设置为 Enable？')">
    <i class="icon-unlock"></i> Enable</a>
    <?php else: endif; ?>
  <?php if($status==0||$status==1): ?><a class="btn btn-default checkbox-opreat disabled" href="javascript:checkDialogs('/Admin/Web/checkDeletes?status=2','确定批量设置为 Disable？')">
    <i class="icon-lock"></i> Disable</a>
    <?php else: endif; ?>
      <a class="btn btn-default checkbox-opreat disabled" href="javascript:checkDialogs('/Admin/Web/checkDeletes','确定删除所选择数据？')">
    ...
    <?php echo ($page); ?> 
      ...
      <div class="table-responsive">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td class="col-md-4 autoline"><a href="<?php echo ($vo["source_link"]); ?>" target="_blank"><?php echo ($vo["title"]); ?></a></td>
                <td class="col-md-1">
                  <?php switch($vo["type"]): case "1": ?><i class="icon-film"></i> 视频<?php break;?>
                    <?php case "2": ?><i class="icon-picture"></i> 图片<?php break;?>
                    <?php case "3": ?><i class="icon-book"></i> 文字<?php break;?>
                    <?php case "4": ?><i class="icon-headphones"></i> 音乐<?php break;?>
                    <?php default: ?>---<?php endswitch;?>
                </td>
                <td class="col-md-3"><?php  $arrTags=array(); $tagStrs=""; $tagStr=$vo["tags"];if($tagStr!=null&&$tagStr!=""){ $arrTags=explode(",", $tagStr); } for ($i=0; $i <count($arrTags); $i++) { $labelType = array("default","primary","info","success","danger","warning"); $typeId=mt_rand(0,5); echo "<span class='label label-".$labelType[$typeId]."'>".$arrTags[$i]."</span> "; }?>
                 </td>
                <td class="col-md-1">&nbsp;
                ...
                <td class="col-md-3">
                   <a class="btn btn-default" href="javascript:toggleModel('/Admin/Web/test');" ><i class="icon-info-sign"></i></a>
                   <a class="btn btn-default" href="/Admin/Web/editinfo?_id=<?php echo ($vo["dbid"]); ?>"><i class="icon-edit"></i></a>
    <?php if($vo["status"] != 1): ?><a class="btn btn-success" href="javascript:checkDialog('/Admin/Web/changeStatus?dbid=<?php echo ($vo["dbid"]); ?>&status=1','确定设置为 Enable？')"><i class="icon-unlock"></i></a><?php endif; ?>
    <?php if($vo["status"] != 2): ?><a class="btn btn-warning" href="javascript:checkDialog('/Admin/Web/changeStatus?dbid=<?php echo ($vo["dbid"]); ?>&status=2','确定设置为 Disable？')"><i class="icon-lock"></i></a><?php endif; ?>
    <a class="btn btn-danger" href="javascript:checkDialog('/Admin/Web/changeStatus?dbid=<?php echo ($vo["dbid"]); ?>&status=2','确定选择删除？')"><i class="icon-trash"></i></a>
                </td>
